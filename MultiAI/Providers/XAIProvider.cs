...
namespace MultiAI.Providers
{
    public class XAIProvider : ILLMProvider
    {
        ...
        private string _model = "grok-2-latest";
        private string _apiKey = string.Empty;
        private const string EndpointUri = "https://api.xai.com/v1";

        public string Name => "xAI (Grok)";

        public void Initialize(string apiKey, string model = "grok-2-latest")
        {
            _apiKey = apiKey;
            _model = string.IsNullOrWhiteSpace(model) ? "grok-2-latest" : model;
        ...
        {
            if (_client == null) return "Error: xAI Provider not initialized with API Key.";
            ...
            [EnumeratorCancellation] CancellationToken cancellationToken = default)
        {
            if (_client == null)
            {
                yield return "Error: xAI Provider not initialized with API Key.";
                yield break;
            }
